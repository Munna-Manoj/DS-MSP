...
whole-file links that 404 on the real site" class of drift a docs/learn/ audit originally found
by hand (since fixed: every link converted to an absolute, line-anchored GitHub URL). Skipped
(not failed) when the optional ``docs`` dependency group isn't installed or ``mkdocs.yml``
doesn't exist, matching this repo's dataset-gated test convention (``tests/realdata/``).
"""
import subprocess
import sys

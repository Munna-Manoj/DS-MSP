...
| UCM | `UCMModel` | 0.334 | lossy — UCM has 1 shape parameter (`alpha`) vs. DS/EUCM's 2 |
...
- **Why two different models can describe the same camera** — the intuition behind
  ...
  [Are two models the same camera?](../explain/are_two_models_the_same_camera.md)
- **The `adapt` API** (`convert`, the report fields, sampling):
  [Multi-model library & conversion](../MULTI_MODEL.md).

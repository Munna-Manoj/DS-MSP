...
Both heuristics fail often enough to quietly ruin a calibration — a model that's perfect for a
GoPro is over-parameterized junk on a webcam, and a model that nails a 160° drone lens can't
bend enough for a 195° security dome.
...
with **KB** itself as the reference.
...
all of these models tie at ~0.08 px.
...
EUCM is beautifully conditioned but (diagnostic A) lacks capacity; EUCM⁺ on the benign lens
has a near-null third direction (cond 5.9e6, smallest singular value ~20 against ~10⁸) — a
parameter that isn't earning its place. **The smallest singular value, not the count, tells
you the effective DOF.**
...
EUCM⁺'s division term λ₁ is ~collinear with EUCM's β on both lenses (residual ≤1.2%): a
structural inefficiency in *that* model's design. DS⁺'s second division term λ₂ keeps 4–13%
genuinely its own — a parameter that pays for itself.
...
kb 10 ms   eucm 3 ms   eucm+ 5 ms   ds+ 22 ms

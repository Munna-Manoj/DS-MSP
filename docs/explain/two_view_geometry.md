...
Two central cameras observe a 3D point. The relative pose $(R, t)$ maps a point from camera 1 to
camera 2:
...
Write $\lambda_i$ for the **depth along the ray**:
$$\lambda_i = \lVert X_i\rVert > 0 .$$
Each point then splits into depth times direction:
$$X_1 = \lambda_1 f_1, \qquad X_2 = \lambda_2 f_2 .$$

## 1. The epipolar constraint holds on rays

**Claim.** The epipolar constraint holds on rays:
$$f_2^\top E\, f_1 = 0, \qquad E = [t]_\times R .$$
Here $[t]_\times$ is the skew matrix defined by
$$[t]_\times v = t \times v .$$

**Proof.** Substitute $\lambda_i f_i$ for $X_i$ in the pose relation:
$$\lambda_2 f_2 = \lambda_1 R f_1 + t .$$
Left-multiply by $f_2^\top [t]_\times$. Because $[t]_\times$ is skew, the term on the left
vanishes:
$$f_2^\top [t]_\times f_2 = 0 .$$
That leaves
$$0 = \lambda_1\, f_2^\top [t]_\times R\, f_1 + f_2^\top [t]_\times t .$$
The last term also vanishes, since
$$[t]_\times t = t \times t = 0 .$$
With nonzero depth $\lambda_1$, divide through:
$$f_2^\top \underbrace{[t]_\times R}_{E}\, f_1 = 0. \qquad\blacksquare$$
Geometrically, $f_2$, $t$, and $R f_1$ are coplanar — they all lie in the epipolar plane. The
scalar triple product states exactly that:
$$f_2 \cdot (t \times R f_1) = 0 .$$
...
**Rank and singular values.** $[t]_\times$ is skew and rank 2, with singular values
$\{\lVert t\rVert, \lVert t\rVert, 0\}$. Right-multiplying by the orthogonal $R$ preserves
singular values, so $E$ inherits the same set — **two equal, one zero**. Hence $E$ is rank 2
with zero determinant:
$$\operatorname{rank} E = 2, \qquad \det E = 0 .$$
The code normalizes the singular values to $(1, 1, 0)$, since scale is unrecoverable.
...
$$a = b, \qquad c = 0 .$$
It is the constraint a minimal 5-point solver enforces directly.
...
Each correspondence gives one linear equation in the 9 entries of $E$. Vectorize $E$ row-major:
$$e = \operatorname{vec}(E) .$$
The epipolar constraint is then linear in $e$:
...
That raw $\hat E$ does not generally satisfy the rank / equal-singular-value structure. Project
it onto the essential manifold. Take the SVD:
...
The third singular value drops to 0 because that is the nearest rank-2 matrix. Fixing $\bar\sigma$
to 1 only rescales, and scale is free. So the projection is the optimal essential-matrix
...
Start from the projected essential matrix, with $U$ and $V$ made proper rotations:
$$E = U\,\mathrm{diag}(1,1,0)\,V^\top, \qquad \det U = \det V = +1 .$$
Flip signs otherwise, so the results are rotations rather than reflections. Define
...
To pick the physical solution, triangulate each correspondence. Keep the candidate whose points
lie **in front of both cameras**.

For a pinhole, the test is
$$z > 0 .$$
For a fisheye that test is **wrong**. A ray past $90°$ has
$$z \le 0 ,$$
yet it is a perfectly valid observation — the same >180° point made in
[Chapter 3](../learn/03_projection_validity.md).

The correct, model-free test is **positive depth *along the bearing vector***:
...
Given the pose $(R, t)$, express both rays in **camera-1** coordinates.

Ray 1 passes through the origin:
$$P(\lambda_1) = \lambda_1 f_1 .$$
Camera 2's centre in the camera-1 frame comes from its own origin ($X_2 = 0$):
$$c_2 = -R^\top t .$$
Ray 2's direction is
$$d_2 = R^\top f_2 ,$$
...
Setting the gradient to zero gives the standard $2\times2$ system. Both $f_1$ and $d_2$ are unit
vectors, so
$$f_1\!\cdot\!f_1 = d_2\!\cdot\!d_2 = 1 .$$
...
$$X = \tfrac12\big(\lambda_1 f_1 + c_2 + \lambda_2 d_2\big) .$$

The denominator equals $\sin^2\theta$, where $\theta$ is the angle between the rays:
$$1 - b^2 = \sin^2\theta .$$
It vanishes only when the rays are **parallel** — zero parallax, or infinite depth — the sole
degeneracy. The recovered point reprojects onto both rays to $\sim0°$.
...
**Conditioning and spherical whitening.** Unit bearing vectors are already $O(1)$ and
well-scaled. So $A$ is far better conditioned than the pixel design matrix the classic
eight-point must Hartley-normalize. Pixel-domain Hartley normalization does **not** transfer to
the sphere.

The analogue — `normalize=True`, the 360-8PA idea — whitens the ray covariance, solves in the
...
up at small $\sigma$ (e.g. $10^{-4}$ rad gives $\sim0.01°$).

*Test:* `test_noise_degrades_gracefully_and_monotonically`.

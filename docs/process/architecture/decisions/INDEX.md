...
| [ADR-0014](ADR-0014-sparse-studentized-irls.md) | Studentized (bounded-influence) IRLS in the sparse Schur solver | Rejected | Sparse implementation proven correct (projector identity, FD parity) but formally proven valueless on this pipeline: robust per-view initialization forecloses the masking regime it targets, studentization slightly *worsens* accuracy even at forced leverage h=0.97, and costs 3.5x wall time at 8 cameras; only the `noise_bound` config fix survives (FR-RIG-019) |
...
| [ADR-0021](ADR-0021-bearing-gnc-tls-pnp.md) | Deterministic bearing-space GNC-TLS PnP with guarded compatibility refinement | Accepted | Replaces random minimal-set sampling in the recommended and rig robust front end while preserving guarded RANSAC compatibility |

...
least-squares on a full-sphere chordal residual.
"""
...
def angular_reprojection_error(f1: np.ndarray, f2: np.ndarray,
    ...
    error metric :func:`refine_two_view` minimizes (see the chordal construction in
...
def _build_two_view_problem(
    ...
    """Build the chordal residual/Jacobian callbacks used by :func:`refine_two_view`.

    Kept as a separate internal constructor so the complete rotation, normalized-translation,
    and per-point structure Jacobian can be finite-difference checked without running an
    optimizer around it.
    """
    f1, f2 = _as_rays(f1), _as_rays(f2)
    n = f1.shape[0]
    ...
    def residual(state):
        """Chordal residual ``(6N,)`` for ``lm_solve``: view 1 then view 2."""
        R, t, X = state
        r1, _P1, _v1 = chordal_bearing_residual_jacobian(X, f1)
        r2, _P2, _v2 = chordal_bearing_residual_jacobian(X @ R.T + t, f2)
        return np.concatenate([r1.ravel(), r2.ravel()])

    def jacobian(state):
        """Analytic ``d(residual)/d(δω, δt, δX)``, shape ``(6N, 6 + 3N)``."""
        R, t, X = state
        # View 1 (camera at identity) sees only X; view 2 sees R·exp(δω)·X + normalize(t+δt).
        J = np.zeros((6 * n, 6 + 3 * n))
        y1 = X
        y2 = X @ R.T + t
        _r1, P1, _v1 = chordal_bearing_residual_jacobian(y1, f1)
        _r2, P2, _v2 = chordal_bearing_residual_jacobian(y2, f2)
        Pt = np.eye(3) - np.outer(t, t)                  # d normalize(t+dt)/d(dt) at dt=0
        for i in range(n):
            J[3 * i:3 * i + 3, 6 + 3 * i:9 + 3 * i] = P1[i]

            G = P2[i]                                    # (3,3): dr2/dy2
            row = 3 * n + 3 * i
            J[row:row + 3, 0:3] = G @ (-R @ hat(X[i]))   # dr2/ddw
            J[row:row + 3, 3:6] = G @ Pt                 # dr2/ddt
            J[row:row + 3, 6 + 3 * i:9 + 3 * i] = G @ R  # dr2/dX_i
        return J

    def retract(state, delta):
        """Manifold update: ``R`` by SO(3) exp, ``t`` re-normalized, ``X`` flat."""
        R, t, X = state
        R = R @ so3_exp(delta[:3])
        t = t + delta[3:6]
    ...
    return (R0, t0, X0), residual, jacobian, retract
...
def refine_two_view(f1: np.ndarray, f2: np.ndarray,
    ...
    """Nonlinear refinement of ``(R, t, X)`` minimizing the chordal angular residual.
    ...
    **Manifold-correct, in-house solver.** Driven by :func:`ds_msp.core.optimize.lm_solve`,
    ...
    The Jacobian is **analytic** (the normalized-bearing difference is differentiated directly),
    ...
    out = lm_solve(state0, residual, jacobian, retract, block=3,
